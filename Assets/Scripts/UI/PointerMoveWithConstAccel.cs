...
/// <summary>
/// 一定間隔ではねるようなアニメーションをし続けるUI用のクラス
/// 方向、距離、周期をInspectorから設定できる
/// </summary>
public class PointerMoveWithConstAccel : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            _defPosition = _rectTransform.anchoredPosition;
        }
    ...
    /// アニメーションの速度の更新
    /// </summary>
    /// <param name="animFirstSpeed">初速度</param>
    /// <param name="animAcceleration">加速度</param>
    /// <param name="normalDir">アニメーション方向</param>
    /// <returns></returns>
    private Vector3 UpdateVelocity(float animFirstSpeed, float animAcceleration, Vector2 normalDir)
    {
        if (_animSpeed <= animFirstSpeed * -1)
        {
            if (_rectTransform == null)
            {
                transform.position = _defPosition;
            }
            else
            {
                _rectTransform.anchoredPosition = _defPosition;
            }
